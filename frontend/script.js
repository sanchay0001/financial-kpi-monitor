const API = "";  // same server — works both locally and on Render
...
function renderTickerTabs(companies) {
  document.getElementById("ticker-tabs").innerHTML = companies.map(c => `
...
function renderHero(company, kpis) {
  ...
  document.getElementById("hero-kpis").innerHTML = items.map(i => {
    const val = i.value != null ? i.value.toFixed(2) + i.suffix : "—";
    const cls = i.value == null ? "" : i.value >= 0 ? "positive" : "negative";
...
function renderKPIGrid(kpis) {
  document.getElementById("kpi-grid").innerHTML = KPI_META.map(([key, label, cat, suffix, hib]) => {
    const raw = kpis[key];
    const valStr = raw != null ? raw.toFixed(2) + suffix : "—";
    const cls    = raw == null ? "null" : hib ? (raw >= 0 ? "good" : "bad") : (raw <= 1 ? "good" : "bad");
...
function renderCharts(trends) {
  ...
  makeChart("chart-margins", {
    ...
    data: { labels: years, datasets: [
      { label: "Gross Margin",     data: trends.map(t => t.gross_margin),     borderColor: "#4f8ef7", tension: 0.3, fill: false },
      { label: "Net Margin",       data: trends.map(t => t.net_margin),       borderColor: "#38d9a9", tension: 0.3, fill: false },
      { label: "Operating Margin", data: trends.map(t => t.operating_margin), borderColor: "#f5a623", tension: 0.3, fill: false },
    ]}, options: CHART_OPTS,
  });
  makeChart("chart-growth", {
    ...
    data: { labels: years, datasets: [{
      ...
      backgroundColor: trends.map(t => (t.revenue_growth_yoy ?? 0) >= 0 ? "#4f8ef760" : "#f0659560"),
      borderColor:     trends.map(t => (t.revenue_growth_yoy ?? 0) >= 0 ? "#4f8ef7"   : "#f06595"),
  ...
  makeChart("chart-roe", {
    ...
    data: { labels: years, datasets: [{
      ...
      borderColor: "#38d9a9", backgroundColor: "#38d9a920", tension: 0.3, fill: true,
    }]}, options: CHART_OPTS,
  });
  makeChart("chart-fcf", {
    type: "line",
    data: { labels: years, datasets: [{
      ...
      borderColor: "#f06595", backgroundColor: "#f0659520", tension: 0.3, fill: true,
    }]}, options: CHART_OPTS,
  });
}
...
function renderCompareTable(data) {
  ...
  const hib  = { gross_margin:true, net_margin:true, revenue_growth_yoy:true,
  ...
  keys.forEach(k => {
    ...
    best[k]  = hib[k] ? Math.max(...vals) : Math.min(...vals);
    worst[k] = hib[k] ? Math.min(...vals) : Math.max(...vals);
  });
  document.getElementById("compare-body").innerHTML = data.map(row => `
    <tr data-ticker="${row.ticker}" onclick="selectTicker('${row.ticker}')">
      ...
      ${keys.map(k => {
        ...
        const txt = v != null ? v.toFixed(2) + (k==="current_ratio"||k==="debt_to_equity" ? "x" : "%") : "—";
